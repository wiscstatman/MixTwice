load("F:/Harddrive-Jul-17-2021/MixTwice follow up/GWAS/mixtwice_result.RData")
load("F:/Harddrive-Jul-17-2021/MixTwice follow up/GWAS/lfdr_all.RData")

xx = rep(1, length(lfdr.summary$unadjust))

xx[ok] = lfdr.mt ## I only care about those lfdr's for unadjust pvalue <= 0.05

lfdr.summary$mixtwice = xx
...
mtfit = ggplot(data = d3, aes(x = grid, y = mix))+
  ...
  labs(x = "grid", y = "cummulative distribution function (cdf)",
       title = "GWAS: Estimated mixing distribution")+
  theme(plot.title = element_text(color = "red", size = 16, hjust = 0),
        strip.text.x = element_text(size = 12, colour = "black", angle = 0),
        legend.position = "top",
        plot.caption = element_text(color = "black", size = 16, face = "italic", hjust = 1),
...
save(rejection, upsetfigure, mtfit, hist_p,
     file = "F:/Harddrive-Jul-17-2021/MixTwice follow up/GWAS/graphic summary.RData")