...
## I will first calculate the mixing distribution of MixTwice
...
mix.mixtwice = get_mixing(thetaHat = bmi$effect_size,
                          s2 = bmi$SE^2,
                          Btheta = 30,
                          Bsigma = 10,
                          df = 100,
...
## then I could calculate the locFDR of mixtwice
...
lfdr.mt = mixtwice_lfdr(thetaHat = bmi$effect_size[ok],
                        s2 = bmi$SE[ok]^2,
                        df = 100,
                        mix.mixtwice = mix.mixtwice)

save(ok,
     lfdr.mt, mix.mixtwice, file = "F:/Harddrive-Jul-17-2021/MixTwice follow up/GWAS/mixtwice_result.RData")
...
ash = get_lfsr(ash(betahat = bmi$effect_size, sebetahat = bmi$SE))

## BL

library(swfdr)

BL = lm_pi0(bmi$pval, X = bmi$ind_covar_N)$pi0 * BH

## AdaPT (GLM)

library(adaptMT)

xx = adapt_glm(x = data.frame(icov = bmi$ind_covar_N), 
               pvals = bmi$pval, 
...
LFDR = clfdr_hickswrapper(unadj_p = bmi$pval,
                                 groups = IHW::groups_by_filter(bmi$ind_covar_N, 20), 
...
save(lfdr.summary,
     file = "F:/Harddrive-Jul-17-2021/MixTwice follow up/GWAS/lfdr_all.RData")
